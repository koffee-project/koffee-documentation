...
\begin{lstlisting}[style=simpleListing, numbers=none]
...
\subsection*{Instruktionen zur Inbetriebnahme der Serveranwendung}
...
\begin{enumerate}
	...
	\item Das benötigten Docker Secret anlegen (siehe \autoref{subsec:backend:configuration}).
...
\begin{enumerate}
	...
	\item Der Server ist nun unter \verb|https://your.domain.org/koffee| erreichbar.
\end{enumerate}
%------------------------------------------------------------------------------------------------------------------------------------------------------
...
\begin{description}
	...
	\item[*POST /users/:id/funding] Lädt das Guthaben des Nutzers mit der gegebenen ID um den gesendeten Betrag auf.
	\begin{itemize}
		\item amount: Double
	\end{itemize}
	...
	\item[GET /users/:id/image] Antwortet mit den Bytes des Profilbilds eines Nutzers mit der gegebenen ID.
